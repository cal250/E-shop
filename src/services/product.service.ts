...
import { CreateProductSchema, UpdateProductSchema, ProductFiltersSchema } from '../validators/product.validators'
...
async getProducts(filters: ProductFiltersSchema): Promise<{ products: ProductResponse[], total: number }> {
    const where: any = {
        deletedAt: null
    }
    ...
    if (filters.minPrice || filters.maxPrice) {
        where.productSkus = {
            some: {
                AND: [
                    filters.minPrice ? { price: { gte: filters.minPrice.toString() } } : {},
                    filters.maxPrice ? { price: { lte: filters.maxPrice.toString() } } : {}
                ]
            }
        }
    }
